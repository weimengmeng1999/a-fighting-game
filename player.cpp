...
void player::isDead()
{
	if (HP <= 0)		// HP less than 0, character is dead
	{
		...
		system("pause");
		playerdeath = 1;	// give the label of death value 1(true)
	}
}
...
void showinfo(player &p1, player &p2)
{
	...
	cout << "# EXP" << setw(3) << p1.EXP << " |Job: " << setw(3);
	p1.showRole();
	cout << "|luck" << setw(3) << p1.bag.nOflu();
	cout << "# EXP" << setw(3) << p2.EXP << " |Job: " << setw(7);
	p2.showRole();
	cout << "|luck" << setw(3) << p2.bag.nOflu();
	cout << "    #" << endl;
	cout << "--------------------------------------------------------------" << endl;
	p1.bag.display();
	cout << "##############################################################" << endl;
